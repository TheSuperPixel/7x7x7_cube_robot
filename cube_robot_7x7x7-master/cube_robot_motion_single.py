...
#test = "U' 3Bw R2 3Bw' 3Lw2 U' 3Fw' 3Lw' 3Dw' 3Rw2 3Fw Uw Bw'"
with open('/home/pi/Desktop/cube_project/cube_robot_7x7x7-master/cube_robot_motion_single_data/cube_robot_motion_single_data.txt', 'r') as file:
    motion_now = file.read()# 读取文件内容并存储到变量中
motion_now = motion_now.strip()#去除从标准输入读取的内容中的前导空格和尾随换行符
motion_now = motion_now.split(',')#逗号进行分割，返回一个包含分割后内容的列表
z_now = int(motion_now[0])
filp_now = int(motion_now[1])
flip_type_now = 1
cube_now = "FL"
exit_flag = False
not_route_flag=0
...
while True:
    ...
    # if exit_flag:
    #     print('exit.')
    with open('/home/pi/Desktop/cube_project/cube_robot_7x7x7-master/cube_robot_motion_single_data/cube_robot_motion_single_data.txt', 'w') as f:#
        f.write(str(z_now)+','+str(filp_now))
    break